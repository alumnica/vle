    ...
    def review_evaluation(self, evaluation, relationship_answers, multiple_option_answers, multiple_answer_answers,
        ...
        for answer_text in answers:
            ...
            relationship_question = [question_instance
                                     ...
                                     if question_instance[1].type == TYPE_RELATIONSHIP
                                     and question_instance[1].pk == int(answer[0])]
            if isinstance(relationship_question[1], RelationShipQuestion):
                ...
                for answer_element in answer[1:]:
                    ...
                    answer_obtained = relationship_question[2][int(answer_element_array[1])]
                ...
                if local_score == len(answer_instance):
                    ...
                    wrong_answers.append([TYPE_RELATIONSHIP, relationship_question[1].pk])
        ...
        for answer_text in answers:
            ...
            multiple_option_question = [question_instance
                                        for question_instance in evaluation
                                        if question_instance[1].type == TYPE_MULTIPLE_OPTION
                                        and question_instance[1].pk == int(answer[0])]
            if isinstance(multiple_option_question[1], MultipleOptionQuestion):
                answer_expected = multiple_option_question[1].correct_answer
                answer_obtained = multiple_option_question[2][int(answer[1])]

                if answer_expected.strip() == answer_obtained.strip():
                    score += 1
                else:
                    wrong_answers.append([TYPE_MULTIPLE_OPTION, multiple_option_question[1].pk])
        ...
        for answer_text in answers:
            ...
            multiple_answer_question = [question_instance
                                        for question_instance in evaluation
                                        if question_instance[1].type == TYPE_MULTIPLE_ANSWER
                                        and question_instance[1].pk == int(answer[0])]
            if isinstance(multiple_answer_question[1], MultipleAnswerQuestion):
                answer_instance = multiple_answer_question[1].correct_answers.split('|')
                answer_element_array = answer[1:]
                for index_obtained in answer_element_array:
                    answer_obtained = multiple_answer_question[2][int(index_obtained)]
                    if answer_obtained in answer_instance:
                        local_score += 1
                if local_score == len(answer_instance):
                    score += 1
                else:
                    wrong_answers.append([TYPE_MULTIPLE_ANSWER, multiple_answer_question[1].pk])
        ...
        for answer_text in answers:
            ...
            numeric_answer_question = [question_instance
                                       for question_instance in evaluation
                                       if question_instance[1].type == TYPE_NUMERIC_ANSWER
                                       and question_instance[1].pk == int(answer[0])]
            if isinstance(numeric_answer_question[1], NumericQuestion):
                answer_obtained = int(answer[1])

                if numeric_answer_question[1].min_limit <= answer_obtained <= numeric_answer_question[1].max_limit:
                    score += 1
                else:
                    wrong_answers.append([TYPE_NUMERIC_ANSWER, numeric_answer_question[1].pk])
        ...
        for answer_text in answers:
            ...
            pulldown_list_question = [question_instance
                                      for question_instance in evaluation
                                      if question_instance[1].type == TYPE_PULL_DOWN_LIST
                                      and question_instance[1].pk == int(answer[0])]
            if isinstance(pulldown_list_question[1], PullDownListQuestion):
                answer_instance = pulldown_list_question[1].answers.split('|')
                for answer_element in answer[1:]:
                    answer_element_array = answer_element.split(',')
                    answer_expected = answer_instance[int(answer_element_array[0])]
                    answer_obtained = pulldown_list_question[2][int(answer_element_array[1])]
                ...
                if local_score == len(answer_instance):
                    ...
                    wrong_answers.append([TYPE_PULL_DOWN_LIST, pulldown_list_question[1].pk])

        progress, created = LearnerEvaluationProgress.objects.get_or_create(learner=learner,
                                                                            evaluation=evaluation[0][1].evaluation)
